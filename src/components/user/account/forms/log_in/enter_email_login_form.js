...
import { logInModal, registerModal, enterPasswordLogInModal }  from 'src/features/user/susiSlice';

import { server } from 'src/axios';

export default function EnterEmailLogInForm() {
  ...
  const handleContinue = async () => {
    ...
    if(valid){
      ...
      if(userExists.data){
        dispatch(enterPasswordLogInModal({email: email}));
      }else{
        setValidEmail('invalid');
        setHelperText('Account doesn\'t exist.');
      }
    }else{
      setValidEmail('invalid');
        ...
        <Box className="a k c b i">
          ...
          <Box className='b c i k' sx={{paddingRight: '12px', marginBottom: '28px', marginTop: '12px'}}>
            <Button onClick={() => dispatch(logInModal())}>
              <Typography sx={{fontFamily: 'mohave', fontSize: 18}}>
                <ArrowBackIcon /> All sign in optons
              </Typography>
            </Button>
          </Box>
          <Button onClick={() => dispatch(registerModal())}>
            <Typography sx={{fontFamily: 'mohave', fontSize: 18, color: 'green'}}>
            Sign up
            </Typography>
          </Button>
        </Box>
